...
# # =====================================
# # 1️⃣ Train Model (Synthetic Dataset)
# # =====================================
...
# st.write("Predict Traffic Level using ANN")

# hour = st.slider("Hour of Day", 0, 23, 8)
# day = st.slider("Day of Week (0=Mon, 6=Sun)", 0, 6, 2)
# temperature = st.slider("Temperature (°C)", 15.0, 45.0, 30.0)
# rain = st.selectbox("Rain", [0, 1])
# holiday = st.selectbox("Holiday", [0, 1])

# if st.button("Predict Traffic"):

#     input_data = scaler.transform([[hour, day, temperature, rain, holiday]])
#     prediction = model.predict(input_data)
...
#     if class_index == 0:
#         st.success(f"🟢 Low Traffic ({prob:.2f}%)")
#     elif class_index == 1:
#         st.warning(f"🟡 Medium Traffic ({prob:.2f}%)")
#     else:
#         st.error(f"🔴 High Traffic ({prob:.2f}%)")
